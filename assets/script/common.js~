...
function IFrameReSize(iframename) {
    ...
    if (pTar) { //ie
        if (!+[1, ] && pTar.document && pTar.document.body.scrollHeight) {
...
function IFrameReSizeWidth(iframename) {
    var pTar = document.getElementById(iframename);
    if (pTar) { //ie
        if (!+[1, ] && pTar.document && pTar.document.body.scrollWidth) {
            //alert(pTar.document.body.scrollWidth);
...
function goTemplate(name) {
    ...
    mainHtml += '            　<iframe src="" scrolling="no" frameborder="0" id="contentDiv" width="100%" onload=\'IFrameReSize("contentDiv");\'></iframe>';
    ...
    $("#contentDiv").load(function () {
    		...
    		if(self){
	    		with($(this).get(0).contentWindow){
					if(self.attr)self.attr("height",	$(document.body).outerHeight());    		
	    		}
    		}
    });
    if (html == '' || html == '#') {
